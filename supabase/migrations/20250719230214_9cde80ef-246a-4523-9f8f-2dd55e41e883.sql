-- Fix the auto-vote function logging reference
...
DECLARE
  ...
  next_turn INTEGER;
  reorder_player_rec RECORD; -- Separate variable for the reordering loop
BEGIN
    ...
    IF player_rec.id IS NOT NULL THEN
      ...
      IF preselection_rec.country_id IS NOT NULL THEN
        ...
        IF game_rec.updated_at < now() - interval '30 seconds' THEN
          ...
          -- 3. Reorder positions for each player individually to avoid conflicts
          -- For each player in the game, renumber their remaining preselections
          FOR reorder_player_rec IN 
            SELECT gp.id as player_id FROM game_players gp WHERE gp.game_id = game_rec.id
          LOOP
            -- Create a temporary table with new positions
            WITH ordered_preselections AS (
              SELECT 
                pp.id,
                ROW_NUMBER() OVER (ORDER BY pp.position) as new_position
              FROM player_preselections pp
              WHERE pp.player_id = reorder_player_rec.player_id
            )
