-- Fix the auto-vote function to ensure proper turn synchronization and prevent race conditions
    ...
    IF player_rec.id IS NOT NULL THEN
      ...
      IF preselection_rec.country_id IS NOT NULL THEN
        ...
        IF game_rec.updated_at < now() - interval '30 seconds' THEN
          ...
          -- 3. Update positions for all remaining preselections in the game
...
-- Add a unique constraint to prevent duplicate country selections (if not already exists)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'game_countries_unique_country_per_game'
  ) THEN
    ALTER TABLE game_countries 
    ADD CONSTRAINT game_countries_unique_country_per_game 
    UNIQUE (game_id, country_id);
  END IF;
END $$;

-- Create an index to optimize turn-based queries
CREATE INDEX IF NOT EXISTS idx_games_turn_lookup 
ON games (game_phase, status, current_player_turn, updated_at);

-- Create an index to optimize preselection queries
CREATE INDEX IF NOT EXISTS idx_player_preselections_lookup 
ON player_preselections (player_id, position, country_id);